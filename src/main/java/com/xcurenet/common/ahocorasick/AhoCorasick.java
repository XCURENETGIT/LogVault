...
public class AhoCorasick {
	 ...
	 */
	public void add(final byte[] keyword, final Object output) {
		if (this.prepared) {
			throw new IllegalStateException("can't add keywords after prepare() is called");
		}
		final State lastState = this.root.extendAll(keyword);
	 ...
	 */
	public Iterator<SearchResult> search(final byte[] bytes) {
	...
	private void prepareFailTransitions() {
		...
		while (!q.isEmpty()) {
			...
			final byte[] keys = state.keys();
			for (int i = 0; i < keys.length; i++) {
				State r = state;
				final byte a = keys[i];
				final State s = r.get(a);
				q.add(s);
				r = r.getFail();
				while (r.get(a) == null) {
					r = r.getFail();
	 ...
	 */
	public SearchResult startSearch(final byte[] bytes) {
		if (!this.prepared) {
	 ...
	 * Package protected.
	 */
	public SearchResult continueSearch(final SearchResult lastResult) {
		final byte[] bytes = lastResult.bytes;
