...
class State {
	...
	private State fail;
	private final Set<Object> outputs;

	public State(final int depth) {
		this.depth = depth;
		if (depth > THRESHOLD_TO_USE_SPARSE) this.edgeList = new SparseEdgeList();
		else this.edgeList = new DenseEdgeList();
		this.fail = null;
		this.outputs = new HashSet<>();
	}

	public State extend(final byte b) {
		if (this.edgeList.get(b) != null) return this.edgeList.get(b);
		final State nextState = new State(this.depth + 1);
		this.edgeList.put(b, nextState);
		return nextState;
	}

	public State extendAll(final byte[] bytes) {
		State state = this;
	...
	public void setFail(final State f) {
		this.fail = f;
	}
