...
public class PropertySourceLoader implements EnvironmentPostProcessor, Ordered {
	...
	private static final String QUERY = "SELECT CONF_ID, NVL(VAL, DEFAULT_VAL) AS VAL FROM UI_CONF WHERE APP_CD = ? AND USE_YN =? " +
            "UNION ALL SELECT CONF_ID, NVL(VAL, DEFAULT_VAL) AS VAL FROM UI_CONF WHERE APP_CD = 'UI' AND USE_YN ='Y' AND CONF_ID = 'user.identification.mode'" +
            "UNION ALL SELECT CONF_ID, NVL(VAL, DEFAULT_VAL) AS VAL FROM UI_CONF WHERE APP_CD = 'UI' AND USE_YN ='Y' AND CONF_ID = 'gs.enable'";
