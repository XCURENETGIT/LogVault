...
public class ServiceLoader {
	...
	public void load() {
		...
		List<ServiceVO> service = infoLoaderService.getService();
		for (ServiceVO item : service) {
			if (Common.isEmpty(item.getServiceCd()) || Common.isEquals(item.getUseYn(), "N") || Common.isEquals(item.getLoggingYn(), "N")) continue;
