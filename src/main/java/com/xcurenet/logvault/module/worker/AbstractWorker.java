...
public abstract class AbstractWorker implements Runnable {
    ...
    protected void process(ScanData data) throws ProcessDataException {
        ...
        log.info("MG_START | {} | {} | ATT:{} | {} | {} | {}", msg.getSourceIp(), msg.getSvc(), msg.getAppFile().size(), msg.getSubject(), conf.getWmailPathSmall(data.getFilePath().toAbsolutePath().toString(), blocked), DateUtils.stop(sw));
    }

    protected void checkAttachments(ScanData data) throws SkipFileException {
        MSGData msg = data.getMsgData();
        boolean blocked = isBlockAction(data);
        if (msg.getMsgFile() != null) checkFiles(data, conf.getPath(msg.getMsgFile(), blocked));

        if (msg.getAppFile() != null) {
            for (String appFile : msg.getAppFile()) {
                checkFiles(data, conf.getPath(appFile, blocked));
            }
        }
    }
    ...
    protected void transToAttach(ScanData data) throws FileSendException {
        ...
        for (String src : msg.getAppFile()) {
            ...
            Path srcPath = Paths.get(conf.getPath(src, isBlockAction(data)));
    ...
    protected void transToBody(ScanData data) throws FileSendException {
        ...
        Path srcPath = Paths.get(conf.getPath(msg.getMsgFile(), isBlockAction(data)));
        if (!Files.exists(srcPath)) return;
