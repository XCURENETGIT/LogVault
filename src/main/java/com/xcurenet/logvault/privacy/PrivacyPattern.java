...
public class PrivacyPattern {
	...
	static {
		registerPattern(PrivateType.SN.name(), "\\b[0-9]{2}(0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])[-!@#$%^*|/?~._\\s]*?[1-4][0-9]{6}\\b", new SNValidator()); // 주민등록번호 (내국인)
	...
	private static void registerPattern(String type, String regex, PatternValidator validator) {
		Expression expression = new Expression(regex, EnumSet.of(ExpressionFlag.SOM_LEFTMOST), nextId);
